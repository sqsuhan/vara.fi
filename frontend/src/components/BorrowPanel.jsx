...
export default function BorrowPanel() {
  ...
  return (
    <div className="glass-panel rounded-3xl p-6 relative overflow-hidden group">
      ...
      <div className="absolute top-[-50px] right-[-50px] w-32 h-32 bg-purple-200/50 blur-[60px] rounded-full pointer-events-none transition-all duration-500 group-hover:bg-purple-300/50"></div>

      {/* Header */}
      <div className="flex items-center gap-4 mb-6 relative z-10">
        <div className="w-12 h-12 rounded-2xl bg-purple-50 border border-purple-100 flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
        ...
        <div>
          <h3 className="text-xl font-black text-gray-900 tracking-tight">Borrow</h3>
          <p className="text-sm text-gray-500 font-medium">Get liquidity against your collateral</p>
        </div>
      </div>
      ...
      <form onSubmit={handleBorrow} className="space-y-4 relative z-10">
        <div className="relative">
          <input
            ...
            disabled={isLoading}
            className="w-full glass-input rounded-2xl px-5 py-4 text-2xl font-bold text-gray-900 placeholder-gray-400 disabled:opacity-50 tracking-tight"
          />
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
            <button
              type="button"
              onClick={setMax}
              className="px-2.5 py-1.5 text-xs font-black bg-purple-50 text-purple-500 hover:bg-purple-100 rounded-lg transition-colors uppercase tracking-widest border border-purple-100"
            >
              Max
            </button>
